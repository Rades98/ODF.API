...
namespace ODF.API.FormFactories
{
	public static class LineupItemFormFactory
	{
		...
		public static Form GetDeleteLineupItemForm(DeleteLineupItemForm form, IEnumerable<ValidationFailure>? errors = null)
		{
			...
			requestForm.AddMember(new("Id položky", nameof(DeleteLineupItemForm.Id), FormValueTypes.Text, form.Id, false,
				errors?.GetErrorMessage(nameof(DeleteLineupItemForm.Id))));

			return requestForm;
		}

		public static Form GetUdpateLineupItemForm(UpdateLineupItemForm form, IEnumerable<ValidationFailure>? errors = null)
		{
			var requestForm = new Form();
			...
			requestForm.AddMember(new("Překladová proměnná popisu", nameof(AddLineupItemForm.DescriptionTranslationCode), FormValueTypes.Text, form.DescriptionTranslationCode, false,
				errors?.GetErrorMessage(nameof(AddLineupItemForm.DescriptionTranslationCode))));
