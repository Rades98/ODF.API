...
namespace ODF.API.Controllers
{
	public class LanguagesController : Controller
	{
		...
		{
			string actionParttext = await _mediator.Send(new GetTranslationQuery("Přepnout do {0}", "app_language_switch", countryCode), cancellationToken);
			...
			string title = await _mediator.Send(new GetTranslationQuery("Jazyk", "app_language", countryCode), cancellationToken);
		...
		public async Task<IActionResult> PostTranslations(string countryCode, [FromBody] ChangeTranslationForm form, CancellationToken cancellationToken)
		{
			...
			bool result = await _mediator.Send(new ModifyTransaltionCommand(form.CountryCode, form.TranslationCode, form.Text), cancellationToken);
