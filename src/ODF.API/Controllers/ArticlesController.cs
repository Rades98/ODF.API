...
namespace ODF.API.Controllers
{
    public class ArticlesController : Controller
    {
        ...
        {
            var result = await _mediator.Send(new AddArticleCommand(model.TitleTranslationCode, model.Title, model.TextTranslationCode, model.Text, model.PageId, countryCode, model.ImageUrl), cancellationToken);
        ...
        public async Task<IActionResult> GetArticle([FromRoute] int articleId, [FromRoute] string countryCode)
        {
            ...
            var notFoundTitle = await _mediator.Send(new GetTranslationQuery("Zdroj nenalezen.", "app_base_notfound", countryCode));
            var notFoundArticle = await _mediator.Send(new GetTranslationQuery("Článek, který se pokoušíte zobrazit, byl nejspíše smazán.", "app_article_notfound", countryCode));
